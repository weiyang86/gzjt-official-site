...
async function loadDetail(id: string) {
  try {
    ...
    const query = new URLSearchParams({ scope: 'news', page: '1', pageSize: '100' });
    ...
    return { article, related: context.items.filter((item) => item.id !== id).slice(0, 3), isFallback: false };
  } catch {
    ...
    return { article, related: context.items.filter((item) => item.id !== id).slice(0, 3), isFallback: true };
  }
}

function labelFor(article: Article) {
  const channel = article.mainChannel || article.main_channel;
  return article.newsSubcategory ? `${channel?.name || '新闻中心'} · ${article.newsSubcategory}` : channel?.name || '新闻中心';
}
...
export default async function NewsDetailPage({ params }: DetailPageProps) {
  ...
  const { article, related, isFallback } = data;
        ...
        <section className="section" aria-label="正文">
          <div className="content">
            <div className="detail reveal is-visible">
              ...
              <div className="meta">发布时间：{formatDate(publishValue)} · {labelFor(article)}{article.source ? ` · ${article.source}` : ''}{isFallback ? ' · 本地示例' : ''}</div>
              ...
              <div style={{ marginTop: 18, display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                <a className="btn btn--primary" href="/news">返回列表</a>
            ...
            <div className="section-head reveal is-visible" style={{ marginTop: 32 }}>
              <div>
                ...
                <p className="lead">以卡片形式承接关联内容。</p>
              </div>
            </div>
            <div className="grid">
              {related.map((item) => (
