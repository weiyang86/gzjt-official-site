...
const getNoticeChannels = async (token: string) => {
  const result = await directusJsonRequest<{ data?: Array<{ id?: string; slug?: string; name?: string }> }>(getChannelsPath('notice'), token);
...
const handleLocalMe = async (request: NextRequest) => {
  ...
  try {
    const me = await directusRequest<{ data?: unknown }>('/users/me?fields=id,email,first_name,last_name,role.id,role.name,role.description', {
...
const handleLocalChannels = async (request: NextRequest) => {
  ...
  const result = await directusJsonRequest<{ data?: unknown[] }>(getChannelsPath(getAdminScope(request)), session.accessToken);
...
const handleLocalArticleList = async (request: NextRequest) => {
  ...
  if (request.method === 'POST') {
    ...
    const result = await directusJsonRequest<{ data?: unknown }>('/items/articles', session.accessToken, 'POST', payload);
  ...
  const result = await directusJsonRequest<{ data?: unknown[]; meta?: unknown }>(buildDirectusPath('/items/articles', params), session.accessToken);
...
const handleLocalArticleRoute = async (request: NextRequest, path: string[]) => {
  ...
  if (!action && request.method === 'GET') {
    const result = await directusJsonRequest<{ data?: unknown }>(getArticleDetailPath(id), session.accessToken);
  ...
  if (action && request.method === 'PATCH') {
    ...
    const result = await updateArticleStatus(id, nextStatus, session.accessToken);
...
const handleLocalCategoryList = async (request: NextRequest) => {
  ...
  if (request.method === 'GET') {
    const result = await directusJsonRequest<{ data?: unknown[] }>(buildCategoryListPath(request), session.accessToken);
    return NextResponse.json({ data: result.data || [] });
  }
  if (request.method === 'POST') {
    const body = await request.json() as Record<string, unknown>;
    const payload = normalizeCategoryInput(body, true, getAdminScope(request));
    const result = await directusJsonRequest<{ data?: unknown }>('/items/channels', session.accessToken, 'POST', payload);
    return NextResponse.json({ data: result.data || null }, { status: 201 });
  }
  return null;
};
...
const handleLocalCategoryRoute = async (request: NextRequest, path: string[]) => {
  ...
  if (!action && request.method === 'GET') {
    const result = await directusJsonRequest<{ data?: unknown }>(getCategoryDetailPath(id), session.accessToken);
    return NextResponse.json({ data: result.data || null });
  }
  ...
  if (!action && request.method === 'PATCH') {
    ...
    const result = await directusJsonRequest<{ data?: unknown }>(`/items/channels/${encodeURIComponent(id)}`, session.accessToken, 'PATCH', payload);
    return NextResponse.json({ data: result.data || null });
  }
  ...
  if (!action && request.method === 'DELETE') {
    ...
    await directusJsonRequest(`/items/channels/${encodeURIComponent(id)}`, session.accessToken, 'DELETE');
    return NextResponse.json({ data: { id, deleted: true } });
  }
