(function () {
  ...
  const renderRows = (articles) => {
    ...
    if (!articles.length) {
      ...
      cell.colSpan = 6;
      cell.textContent = '暂无公示公告，请点击“新增公示公告”创建。';
    ...
    articles.forEach((article) => {
      ...
      titleCell.textContent = article.title || '未命名公示公告';
      ...
      editLink.href = `/admin/notice-edit.html?id=${encodeURIComponent(article.id)}`;
  ...
  const loadArticles = async () => {
    ...
    try {
      ...
      showMessage(error.message || '公示公告列表加载失败', 'error');
  ...
  articlesBody.addEventListener('click', async (event) => {
    ...
    const confirmText = action === 'archive' ? '确认归档这条公示公告？' : '确认修改公示公告状态？';
    if (!window.confirm(confirmText)) return;
    button.disabled = true;
    try {
      await window.AdminApi.setArticleStatus(id, action, { scope: 'notice' });
